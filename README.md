...
0~3부에서 정적인 구조와 코드를 읽는 기초를 익히고, 4부에서 실행 중인 프로그램을 멈춰 PC·register·Memory·disassembly를 관찰하는 방법을 배웠습니다. 5부에서는 이 관찰 방법을 실제 게임에 적용해 화면 변화에서 조사 대상을 고르고, Data가 어떤 Code를 거쳐 어디에 기록되는지 실행 로그로 확인합니다.

```text
4부  Debugger에서 실행 상태와 사건을 관찰한다
        ↓
5부  화면 변화에서 시작해 Data와 Code의 흐름을 연결하고 다음 조사 위치를 정한다
        ↓
6부  문자·Font·화면 출력 경로를 분석한다
        ↓
7부  처음 보는 게임의 관찰 순서와 분석 범위를 정한다
```

이 과정에서 외워야 할 것은 특정 게임의 주소나 명령이 아닙니다. 화면에서 질문을 만들고, PC·register·Memory·CALL/RET·READ/WRITE의 관계를 근거로 확인하면서 다음 조사 방법을 선택하는 절차를 익힙니다.
